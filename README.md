...
The application's files are cached for offline usage together with the content. In order to pass the offline mode check in [Chrome Lighthouse](https://chrome.google.com/webstore/detail/lighthouse/blipmdconlkpinefehnmjammfjpmpbjk?hl=en), it is enough to cache the app shell (the application's files) and inform the user that the app is currently offline.
...
For engaging your users through web push notifications, we recommend using [OneSignal](https://onesignal.com/). Web push notifications have been proven to increase user engagement by up to 4x.

** *Offline mode and Web Push Notifications are implemented using service workers, so they are currently available on Chrome.*

### Infinite Scrolling for Categories and Products Lists
